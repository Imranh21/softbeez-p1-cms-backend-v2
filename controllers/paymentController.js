...
exports.getUnpaidCustomers = async (req, res) => {
  try {
    ...
    // Find all customers associated with the business
    const customers = await Customer.find({
      "businesses.businessId": new mongoose.Types.ObjectId(businessId),
    });

    // Find all payments made for the specified month and year
    const payments = await Payment.find({
      businessId: new mongoose.Types.ObjectId(businessId),
      dueDate: { $gte: startDate, $lte: endDate },
    });

    // Create a set of customer IDs who have paid
    const paidCustomerIds = new Set(
      payments.map((payment) => payment.customerId.toString())
    );

    // Filter customers who haven't paid
